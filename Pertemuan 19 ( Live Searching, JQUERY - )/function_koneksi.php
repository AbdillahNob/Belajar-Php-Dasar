...
function regis($login){
    global $con;
    // stripslashes adalah fungsi agar tanda(/) bisa di masukkan dlm DATABASE
    $username = strtolower(stripslashes($login["username"]));
    // mysqli_real_escape_string adalah fungsi yg membuat nilai tanda ' bisa diterima DATABASE
    $password = mysqli_real_escape_string($con,$login["password"]);
    $password2 = mysqli_real_escape_string($con,$login["password2"]);

    // cek apakah konfirmasi password sesuai dgn password
    ...
    // cek apakah username yg di input sama dgn username yg ad di DATABASE
    $result = mysqli_query($con, "SELECT username FROM registrasi WHERE username = '$username' ");
    if( mysqli_fetch_assoc($result)){
        echo 
            "<script>
                alert('username yang anda masukkan sudah ada sebelumnya !');
            </script>";
